...
export default function Header() {
  ...
  const [activeLink, setActiveLink] = useState(
    currentLink ? currentLink.name : links[0].name,
  );

  return (
    <header>
      <nav className="bg-white border-gray-200 px-4 py-2.5 dark:bg-gray-800 shadow">
                  ...
                  <Link
                    ...
                    className={`block py-2 pr-4 pl-3 text-gray-700 ${
                      activeLink === link.name
                        ? "rounded md:bg-transparent md:text-primary-700 md:p-0 dark:text-gray-700 md:dark:text-white"
                        : "hover:bg-gray-50 md:hover:bg-transparent md:hover:text-primary-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent"
                    }`}
                  >
                    {link.name}
                  </Link>
